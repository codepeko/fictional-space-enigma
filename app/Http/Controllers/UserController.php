...
class UserController extends Controller
{
    ...
    public function pastes(Request $request)
    {
        ...
        $pastes = Paste::with('language')->where('user_id', \Auth::user()->id)->orderBy('created_at', 'DESC');
    ...
    public function profile($username, Request $request)
    {
        ...
        $pastes = Paste::with('language')->where('user_id', $user->id)->where('status', 1)->where(function ($query) {
    ...
    public function pasteSettingsShow()
    {
        ...
        $paste->short_service = (!empty(\Auth::user()->default_paste->short_service)) ? \Auth::user()->default_paste->short_service : "";
        $paste->short_api_token = (!empty(\Auth::user()->default_paste->short_api_token)) ? \Auth::user()->default_paste->short_api_token : "";
    ...
    public function pasteSettings(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'short_service' => 'required_if:enable_short,=,1|in:ouo.io,exe.io,fc.lc,link.codepeko.com,clk.sh,adshort.co,tmearn,uii.io,mitly.us,adbull.me,short.pe,cpmlink.net,shrtfly,cutwin,short.am,short.es,adf.ly,al.ly,shrinkme.io',
            'short_api_token' => 'required_if:enable_short,=,1|max:255',            
        ]);
        if ($validator->fails()) {
            return redirect()->back()
        ...
        $paste->enable_short = (!empty($request->enable_short)) ? 1 : 0;
        $paste->short_service = $request->short_service;
        $paste->short_api_token = $request->short_api_token;
