...
class Paste extends Model
{
    ...
    public static function boot()
    {
        ...
        static::deleting(function($item) {
            ...
            if ($item->storage == 2) {
                if (file_exists(ltrim($item->content, '/'))) {
                    unlink(ltrim($item->content, '/'));
                }
            }
        });
    } 
    ...
    public function getSlugOrTitleAttribute()
    {
        ...
        if (config('settings.paste_slug_type') == 'title') {
            if(empty(str_slug($this->title))){
                return __('Untitled');
            }

            return str_pad(str_slug($this->title), 10, ' '.mt_rand(10000,99999), STR_PAD_RIGHT);
        }
        else{
